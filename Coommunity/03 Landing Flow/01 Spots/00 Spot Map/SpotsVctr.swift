...
class SpotPOIItem: NSObject, GMUClusterItem {
...
let kSpotClusterItemCount = 50
let kSpotCameraLatitude = -33.8
let kSpotCameraLongitude = 151.2

class SpotsVctr: UIViewController , GMUClusterManagerDelegate, GMSMapViewDelegate,UICollectionViewDelegateFlowLayout,UICollectionViewDelegate,UICollectionViewDataSource {
...
//        print(something)
    ...
    func mapView(_ mapView: GMSMapView, didTap marker: GMSMarker) -> Bool {
        if let poiItem = marker.userData as? SpotPOIItem {
    ...
    private func generateClusterItems() {
        ...
        for index in 1...kClusterItemCount {
            let lat = kSpotCameraLatitude + extent * randomScale()
            let lng = kSpotCameraLongitude + extent * randomScale()
            let name = "Item \(index)"
            let item = SpotPOIItem(position: CLLocationCoordinate2DMake(lat, lng), name: name)
